...
| 項目 | 内容 |
| --- | --- |
| 文書名 | English Reading Quest 要件定義書・基本設計書 |
| 対象システム | TOEIC Part 7形式 英語長文学習Webアプリ |
| 対象読者 | 開発メンバー、レビュー担当者、講師、将来の保守担当者 |
| 文書の目的 | 提供価値と実装済み仕様を共有し、テスト・保守・機能追加時の判断基準とする |
| 開発形態 | 2人チーム、9日間開発 |
| 文書管理 | Markdownをソースコードと同じGitリポジトリで管理する |
...
通常学習では、ユーザーが難易度と文章形式を選択し、英語長文1つと4択問題2問に回答する。採点後は、得点、正誤、正解、本文中の根拠、日本語解説、全文の日本語訳を確認できる。さらに、本文中の気になる1文を選択し、文法構造・語句・フレーズの詳細説明をAIから1回だけ取得できる。
...
| FR-19 | AI通信に失敗した場合は利用回数を消費せず再試行できること | Must |
...
| FR-34 | 問題生成中は画面全体をぼかし、他の操作を抑止してLoadingとキャンセルを表示すること | Must |
| FR-35 | キャンセル時は完了通知を出さず、通常学習は設定画面、タイムアタックはホームへ戻ること | Should |
...
- 詳細説明は各パッセージで新規生成1回、取得済み内容の再表示は可能である。
...
| NFR-10 | 整合性 | 共有メモリ更新をロックで保護する | Must |
...
- DBへの問題・履歴・成績の永続保存
- 複数端末間のデータ共有
- ランキング、SNS共有、決済、管理者画面
- TOEIC Part 1～6
- 複数文書形式のPart 7
- 音声認識、音声読み上げ
- 公式TOEIC問題の収録
- ブラウザを閉じた後も残る長期的なレベル・履歴
...
- フロントの生成キャンセルはHTTP要求の待機を中止するが、送信済みのLLM処理自体がプロバイダー側で直ちに停止するとは限らない。
...
| 詳細説明 | detailedExplanationsBySet、selectedSentencesBySet、detailConfirmationOpen、detailModalOpen |
...
- 同一パッセージの新規生成は1回だけ。
- 同時クリック対策として、LLM通信前にprocessing状態を確保する。
